...
public struct Country: Decodable {
    ...
    public static func getCountry(from dialCode: String) -> Country? {
        return CountriesFetcher().fetch().first(where: { $0.dialCode ==  dialCode})
    }
    
    public static func getCountryFromISOCode(from code: String?) -> Country? {
        return CountriesFetcher().fetch().first(where: { $0.code ==  code ?? Country.getCurrentCountry()?.code})
    }
}
